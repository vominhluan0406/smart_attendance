...
func SafeMigrate(db *gorm.DB, targetModels ...interface{}) error {
	...
	columns := []string{"face_verified", "nfc_verified", "password_verified"}
...
func RawMigrateTurso(db *gorm.DB) error {
	...
	// We ignore the error because ALTER TABLE will fail if the column already exists.
	log.Printf("[database] Turso: ensuring new columns exist")
	_ = db.Exec("ALTER TABLE branches ADD COLUMN beacon_uuid TEXT")
	
	tablesToUpdate := []string{"attendances", "attendance_logs"}
	colsToUpdate := []string{"face_verified", "nfc_verified", "password_verified"}
	for _, t := range tablesToUpdate {
		for _, c := range colsToUpdate {
			_ = db.Exec(fmt.Sprintf("ALTER TABLE %s ADD COLUMN %s INTEGER DEFAULT 0", t, c))
		}
	}

	tables := []string{
		`CREATE TABLE IF NOT EXISTS branches (id TEXT PRIMARY KEY, created_at DATETIME, updated_at DATETIME, deleted_at DATETIME, name TEXT NOT NULL, address TEXT, lat REAL, lng REAL, radius_m INTEGER DEFAULT 200, totp_secret TEXT, beacon_uuid TEXT, allowed_methods TEXT NOT NULL DEFAULT 'qr_totp,ip,location,password', work_start_time TEXT DEFAULT '08:00', work_end_time TEXT DEFAULT '17:00', is_active INTEGER DEFAULT 1)`,
