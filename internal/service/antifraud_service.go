...
var (
	ErrGPSAccuracyTooLow  = fmt.Errorf("độ chính xác GPS quá thấp, nghi ngờ giả mạo vị trí")
	ErrGPSAccuracyTooHigh = fmt.Errorf("độ chính xác GPS quá cao (>150m), vị trí không đáng tin cậy")
	ErrTOTPAlreadyUsed     = fmt.Errorf("mã QR đã được sử dụng, vui lòng đợi mã mới")
	ErrImpossibleTravel    = fmt.Errorf("phát hiện di chuyển bất thường, vui lòng thử lại")
	ErrDeviceBlocked       = fmt.Errorf("thiết bị này đã bị chặn, vui lòng liên hệ quản lý")
	ErrIPLocationMismatch  = fmt.Errorf("vị trí IP và GPS không khớp, nghi ngờ sử dụng VPN")
)
...
func (s *AntiFraudService) CheckIPLocationConsistency(ip string, lat, lng float64, branchLat, branchLng float64) error {
	...
	// A more advanced implementation would use a GeoIP database.
	// For now, we log a warning if GPS and branch are far apart but IP check passed,
	// which could indicate VPN usage.
...
func (s *AntiFraudService) CreateAlert(userID, branchID string, alertType models.FraudAlertType, severity, description string, details map[string]interface{}, ip string, lat, lng *float64) {
