...
import LeaveManageTable from "./leave-manage-table";
...
export default async function ManageLeavePage({
  ...
  try {
    const res = await apiGet<LeaveRequest[]>(
      `/api/leave/manage?status=${statusFilter}&limit=50`,
      cookie
    );
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
          <div>
            ...
            <p className="mt-2 text-lg text-gray-500">
              Xét duyệt các yêu cầu nghỉ phép của nhân viên
            </p>
          </div>
